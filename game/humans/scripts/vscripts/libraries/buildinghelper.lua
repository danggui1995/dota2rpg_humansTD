...
function BuildingHelper:Init()
    ...
    LinkLuaModifier("modifier_xiaotou_baijia", "libraries/modifiers/modifier_xiaotou_baijia", LUA_MODIFIER_MOTION_NONE)
    LinkLuaModifier("modifier_ltx_cleave_attack", "ability/modifier/modifier_ltx_cleave_attack", LUA_MODIFIER_MOTION_NONE)
...
function BuildingHelper:BuildCommand(args)
    ...
    BuildingHelper:AddToQueue(builder, location, queue,args['gold'],args['food'],args['wood'])
end
...
function BuildingHelper:AddBuilding(keys)
    ...
    local _gold = ability:GetLevelSpecialValueFor("gold",0)
    local _food = ability:GetLevelSpecialValueFor("food",0)
    local _wood = ability:GetLevelSpecialValueFor("wood",0)
    ...
    local mgd = BuildingHelper:GetOrCreateDummy(unitName,builder:GetTeamNumber())
...
function BuildingHelper:UpgradeBuilding(building, newName)
    ...
    local __gold = building.gold
    local __food = building.food
    local __wood = building.wood
    ...
    local forwardAngle = building:GetAnglesAsVector()
    ...
    newBuilding:AddAbility("common_gx")
    newBuilding.gold = __gold
    newBuilding.food = __food
    newBuilding.wood = __wood
   
    for i=0,7 do
        local ___ability__ = newBuilding:GetAbilityByIndex(i)
        if ___ability__ then
            local behave = ___ability__:GetBehavior()
            if bit.band(behave,4096)~=0 then
                if not ___ability__:GetAutoCastState() then
                    ___ability__:ToggleAutoCast()
                end
            end
        end
    end
    local myunits = FindUnitsInRadius(newBuilding:GetTeamNumber(), newBuilding:GetAbsOrigin(), nil,18000,
                    DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_BASIC , 0, 0, false)
    for k,v in pairs(myunits) do
        if v:GetOwnerEntity()==building then
            v:SetOwner(newBuilding)
        end
    end
    local __ability2 = newBuilding:FindAbilityByName("common_gx")
    __ability2:SetLevel(1)
    local __len = string.len(newName)
    if string.sub(newName,__len-1,__len-1)=='_' then
        local ___last = tonumber(string.sub(newName,__len,__len))
        local ___type = getTypeOfA(newName)
        if ___type==12 then
            newBuilding:SetRenderColor(200, 50, 50)
        elseif ___type==13 then
            newBuilding:SetRenderColor(50, 50, 200)
        elseif ___type==14 then
            newBuilding:SetRenderColor(50, 250, 50)
        end
        if (___type <=6 and ___last==3) then
            BHModifyJZState(playerID,(___type+4),1)
        end
        if (___type==9 and ___last == 3) then
            BHModifyJZState(playerID,11,1)
        end

        if (___type==7 or ___type==8 or ___type==10 ) and ___last==3 then
            if GameRules.jinjie[playerID+1]["_4"]<=0 then
                local ____ability = newBuilding:FindAbilityByName("upgradeTolv4")
                ____ability:SetLevel(0)
            else
                local ____ability = newBuilding:FindAbilityByName("upgradeTolv4")
                ____ability:SetLevel(1)
            end
        end
    end
    -- Update visuals
    ...
    newBuilding:SetAngles(forwardAngle.x,forwardAngle.y, forwardAngle.z)
    EmitSoundOn("ui.trophy_levelup", newBuilding)
    ...
    --print(newBuilding.construction_size)
...
function BuildingHelper:AddToQueue(builder, location, bQueued,gold,food,wood)
    local playerID = builder:GetMainControllingPlayer()
    local player = PlayerResource:GetPlayer(playerID)
    local playerTable = BuildingHelper:GetPlayerTable(playerID)
    local buildingName = playerTable.activeBuilding
    ...
    if builder:GetUnitName() == buildingName then
        ...
        if overrideGhost then
            ...
            entity = BuildingHelper:GetOrCreateDummy(unitName,builder:GetTeamNumber())
        else
            ...
            entity.wood = wood
            for i=0,7 do
                local ___ability__ = entity:GetAbilityByIndex(i)
                if ___ability__ then
                    local behave = ___ability__:GetBehavior()
                    if bit.band(behave,4096)~=0 then
                        if not ___ability__:GetAutoCastState() then
            ...
            --print(entity.gold..unitName..entity:GetEntityIndex())
            
            local __len = string.len(unitName)
            local __type = string.sub(unitName,__len-1,__len)
            if __type == "11" then
                GameRules.jinjie[playerID+1]["_4"] =  GameRules.jinjie[playerID+1]["_4"] + 1
                local targets1 = FindUnitsInRadius(builder:GetTeamNumber(), builder:GetAbsOrigin(), nil,18000,
                    DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_BASIC , 0, 0, false)
                for k,v in pairs(targets1) do
                    local __unitname = v:GetUnitName()
                    if __unitname==GameRules.role[playerID+1].."7_3" or __unitname==GameRules.role[playerID+1].."8_3" or __unitname==GameRules.role[playerID+1].."10_3" then
                        if v:GetOwnerEntity() == builder then
            ...
            local __qq = getTypeOfA(unitName)
            local __ww = tostring(__qq)
            local __bj = string.sub(unitName,__len-1,__len-1)
            
            entity:SetOwner(builder)

            entity:AddAbility("common_gx")
            local ability2 = entity:FindAbilityByName("common_gx")
            ability2:SetLevel(1)
...
function BuildingHelper:GetOrCreateDummy(unitName,teamnumber)
    if BuildingHelper.Dummies[unitName] then
        return BuildingHelper.Dummies[unitName]
    else
        BuildingHelper:print("AddBuilding "..unitName)
        local mgd = CreateUnitByName(unitName, Vector(0,0,0), false, nil, nil, teamnumber)
...
function BHModifyJZState( id,target,opt )
