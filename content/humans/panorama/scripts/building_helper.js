...
function StartBuildingHelper( params )
{
    ...
    {   
        ...
        //(!GameUI.IsShiftDown() && pressedShift) ||
        if (  !Entities.IsAlive( builderIndex ) )
        {
            EndBuildingHelper();
        }
    }
...
function SendBuildCommand( params )
{
    ...
    var GamePos = Game.ScreenXYToWorld(mPos[0], mPos[1]);
    //, "Queue" : pressedShift
    GameEvents.SendCustomGameEventToServer( "building_helper_build_command", { "builder": mainSelected, "X" : GamePos[0], "Y" : GamePos[1], "Z" : GamePos[2] ,"gold":gold,"food":food,"wood":wood } );
